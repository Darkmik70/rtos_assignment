/*
 * simple_driver.c -- a simple module
 */
...
int simple_init_module(void)
{
    ...
    if (simple_major)
    { // the major number is given as a parameter
        dev = MKDEV(simple_major, simple_minor);
        result = register_chrdev_region(dev, 1, "simple_driver");
    }
    else
    { // otherwise
        result = alloc_chrdev_region(&dev, simple_minor, 1, "simple_driver");
